...
| Compliance | The OmniShift compliance rule set (SEC 206(4)-1 baseline; the firm's lexicon in the library the compliance officer manages) applied to every draft, every save and every send: block, flag for a principal, or auto-correct with a re-read prompt | `complianceCheck` |
